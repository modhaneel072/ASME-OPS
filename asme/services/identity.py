...
def hash_reset_token(token: str) -> str:
    """Digest stored in ``password_reset_tokens.token``. The bearer token itself
    only ever exists in the link sent to its owner, so a database read or a
    backup does not hand out working reset links."""
    return hashlib.sha256((token or "").strip().encode("utf-8")).hexdigest()


def create_password_reset(user, hours=None) -> str:
    """Store a new reset credential for ``user`` and return the bearer token."""
    hours = hours or settings().password_reset_hours
    token = secrets.token_urlsafe(32)
    db.session.add(
        PasswordResetToken(user_id=user.id, token=hash_reset_token(token), expires_at=datetime.utcnow() + timedelta(hours=hours))
    )
    db.session.commit()
    return token
...
def find_valid_reset(token):
    if not (token or "").strip():
        return None
    reset_row = PasswordResetToken.query.filter_by(token=hash_reset_token(token)).first()
    if (
        not reset_row
...
def admin_invite_link_token(user, actor) -> str:
    token = secrets.token_urlsafe(32)
    db.session.add(PasswordResetToken(user_id=user.id, token=hash_reset_token(token), expires_at=datetime.utcnow() + timedelta(hours=72)))
