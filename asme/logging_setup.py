...
# URL variables whose values are credentials. Their values are replaced in the
# logged path so a bearer token never reaches log storage.
SECRET_VIEW_ARGS = frozenset({"token"})


def loggable_path() -> str:
    path = request.path
    for key, value in (request.view_args or {}).items():
        if key in SECRET_VIEW_ARGS and value:
