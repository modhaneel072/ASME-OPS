...
AUTH_SESSION_CREDENTIAL_KEY = "auth_credential"
APP_LOGIN_PATH = "/app/auth/login"
...
def credential_fingerprint(user) -> str:
    """Short keyed digest of the account's current password hash.

    Stored in the session at sign-in and compared on every request, so setting
    a new password (reset link, change password, administrator reset) ends
    every session that was opened under the old one. Keyed with the secret key
    because Flask session cookies are signed, not encrypted."""
    secret = (settings().secret_key or "").encode("utf-8")
    material = f"{user.id}:{user.password_hash or ''}".encode("utf-8")
    return hmac.new(secret, material, hashlib.sha256).hexdigest()[:32]


def refresh_session_credential(user) -> None:
    """Keep the caller's own session valid after they change their password."""
    session[AUTH_SESSION_CREDENTIAL_KEY] = credential_fingerprint(user)
...
def current_auth_user():
    ...
    if not user or not user.is_active:
        sign_out_user()
        return None
    if not hmac.compare_digest(str(session.get(AUTH_SESSION_CREDENTIAL_KEY) or ""), credential_fingerprint(user)):
        sign_out_user()
        return None
...
def _deny_login():
    """401 JSON for API clients; browsers go to the ASME Ops sign-in with ``next``."""
    if _wants_json():
        return jsonify({"ok": False, "code": "login_required", "error": "Login required."}), 401
    return redirect(f"{APP_LOGIN_PATH}?{urlencode({'next': request.path})}")
