...
Updated: 2026-09-08 (note added 2026-09-14). Complements [ADR-0001](decisions/ADR-0001-transitional-architecture.md).

Removing the public site, portal, kiosk and legacy HTML sign-in pages (2026-09-14) did not change the schema: every legacy table below still exists and keeps its data. The "portal" and "kiosk" notes describe where the data came from, not screens that still exist.
...
`ensure_membership(user)` also runs at sign-in so users created outside ASME Ops (accounts from the removed signup page, legacy roster import) get a membership on first login.
