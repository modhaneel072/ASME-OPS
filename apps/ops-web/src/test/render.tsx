 ...
 * or a function returning it; return `{ __error: { status, code, error, errors } }`
...
export function mockApi(handlers: Record<string, MockHandler>) {
  ...
  const spy = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
    ...
    const failure = (result as { __error?: { status: number; code: string; error: string; errors?: Record<string, string> } } | null)?.__error
    if (failure) {
      return new Response(JSON.stringify({ ok: false, code: failure.code, error: failure.error, errors: failure.errors }), { status: failure.status, headers: { 'content-type': 'application/json' } })
    }
