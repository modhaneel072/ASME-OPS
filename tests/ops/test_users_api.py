...
def test_invite_new_user_creates_login_membership_and_link(client, org, users, api_login, captured_events):
    ...
    assert "/reset-password/" in payload["invite_url"] and payload["invite_url"].startswith("http")
    reset = PasswordResetToken.query.filter_by(user_id=user.id).one()
    assert reset.token == token and reset.used_at is None
    ...
    done = client.post(f"/reset-password/{token}", data={"password": "brand-new-secret", "confirm_password": "brand-new-secret"})
