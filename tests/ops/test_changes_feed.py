...
def test_feed_lists_visible_events_oldest_first_with_hrefs(client, org, users, ctx_admin, api_login):
    ...
    assert set(events[0]) == {"id", "event_type", "entity_type", "entity_id", "actor", "summary", "before", "after", "metadata", "occurred_at", "href"}
    ...
    assert events[3]["href"] is None and events[3]["before"] == {"name": "x"} and events[3]["after"] == {"name": "Shop"}
