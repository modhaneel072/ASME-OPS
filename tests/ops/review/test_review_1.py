...
def test_inviting_existing_account_does_not_mint_reset_token_for_inviter(client, org, users, api_login):
    ...
    live_tokens = [
        ...
        if row.used_at is None and row.expires_at > datetime.utcnow()
    ]
    ...
    if invite_url:
        ...
        token = (parse_qs(parsed.fragment or parsed.query).get("token") or [""])[0]
        client.post(
            "/api/v1/auth/reset-password",
            json={"token": token, "password": "hijacked-secret", "confirm_password": "hijacked-secret"},
        )
